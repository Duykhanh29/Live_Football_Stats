...
void main() {
  late GetMatchesOfLeagueUseCase getMatchUseCase;
  late MockMatchRepositories mockMatchRepositories;
  setUp(() {
    mockMatchRepositories = MockMatchRepositories();
    getMatchUseCase =
        GetMatchesOfLeagueUseCase(matchRepositories: mockMatchRepositories);
  });
  ...
  test("should get match from repository", () async {
    // // arrange
    // when(mockMatchRepositories.getListMacthesByLeagueID(testId))
    //     .thenAnswer((realInvocation) async => Right(list));
    // //act
    // final result = await getMatchUseCase.call(testId);
    // // assert
    // expect(result, Right(list));
    // verify(mockMatchRepositories.getListMacthesByLeagueID(testId));
