...
void main() {
  late GetListLiveMatchesOfAStageUseCase getListLiveMatchesOfAStageUseCase;
  late MockLiveScoreRepositories mockLiveScoreRepositories;
  setUp(() {
    mockLiveScoreRepositories = MockLiveScoreRepositories();
    getListLiveMatchesOfAStageUseCase = GetListLiveMatchesOfAStageUseCase(
        liveScoreRepositories: mockLiveScoreRepositories);
  });
  ...
  test("should get all stage from repository", () async {
    // arrange
    when(mockLiveScoreRepositories.getListLiveMatchesOfAStage(
            testLeagueID, testStageID))
        .thenAnswer((realInvocation) async => Right(listMatch));
    //act
    final result =
        await getListLiveMatchesOfAStageUseCase.call(testLeagueID, testStageID);
    // assert
    expect(result, Right(listMatch));
    verify(mockLiveScoreRepositories.getListLiveMatchesOfAStage(
        testLeagueID, testStageID));
